"""
Constants for icons and text
"""

"""ICONS"""


class Icons:
    APPS_FIND_MOBILE_APPS_BUTTON = 'apps_find_mobile_apps_button'
    APPS_SIRIUSXM_BUTTON = 'apps_siriusxm_button'
    ARROW_DOWN_BUTTON_ACTIVE = 'arrow_down_button_active'
    ARROW_DOWN_BUTTON_INACTIVE = 'arrow_down_button_inactive'
    ARROW_DOWN_SMALL_BUTTON = 'arrow_down_small_button'
    ARROW_UP_BUTTON_INACTIVE = 'arrow_up_button_inactive'
    AUDIO_DIRECT_ENTER_BUTTON_ACTIVE = 'audio_direct_enter_button_active'
    AUDIO_FM_SOURCE_BUTTON = 'audio_fm_sources_button'
    AUDIO_FREQUENCY_BUTTON = 'audio_frequency_button'
    AUDIO_PRESETS_BUTTON = 'audio_presets_button'
    AUDIO_SIRIUS_SOURCES_BUTTON = 'audio_sirius_sources_button'
    BACK_BUTTON = 'back'
    BACK_PROPERTY = 'back_property_field'
    BLUETOOTH_SWITCH_ON_BUTTON = 'switch_on'
    CHECKED_SETTINGS_NAVIGATION_CHECKBOX_BUTTON = 'checkbox_checked'
    CLIMATE_A_C_BUTTON_ACTIVE = 'climate_a_c_button_active'
    CLIMATE_ARROW_DOWN_BUTTON = 'climate_arrow_down_button'
    CLIMATE_ARROW_UP_BUTTON = 'climate_arrow_up_button'
    CLIMATE_AUTO_BUTTON_ACTIVE = 'climate_auto_button_active'
    CLIMATE_AUTO_BUTTON_INACTIVE = 'climate_auto_button_inactive'
    CLIMATE_DEFROST_BUTTON = 'climate_defrost_button'
    CLIMATE_DEFROST_FRONT_HEATING_BUTTON_ACTIVE = 'climate_defrost_front_heating_button_active'
    CLIMATE_DEFROST_MAX_FRONT_HEATING_BUTTON_INACTIVE = 'climate_defrost_max_front_heating_button_inactive'
    CLIMATE_DEFROST_REAR_HEATING_BUTTON_ACTIVE = 'climate_defrost_rear_heating_button_active'
    CLIMATE_DOWN_AIRFLOW_BUTTON_ACTIVE = 'climate_down_airflow_button_active'
    CLIMATE_DOWN_AIRFLOW_BUTTON_INACTIVE = 'climate_down_airflow_button_inactive'
    CLIMATE_ON_OFF_BUTTON_ACTIVE = 'climate_on_off_button_active'
    CLIMATE_TOP_AIRFLOW_BUTTON_ACTIVE = 'climate_top_airflow_button_active'
    CLIMATE_TOP_AIRFLOW_BUTTON_INACTIVE = 'climate_top_airflow_button_inactive'
    CLOCK_SWITCH_OFF = 'switch_off'
    CLOSE_BUTTON = 'close'
    DEFROST = 'defrost'
    HOME_EXCLAMATION_ICON = 'home_exclamation_icon'
    HOME_PAGE_ICON = 'home'
    HOME_SIRIUS_ICON = 'home_sirius_icon'
    INFO_BUTTON = 'info_button'
    MAIN_APPS_BUTTON_ACTIVE = 'main_apps_button_active'
    MAIN_APPS_BUTTON_INACTIVE = 'main_apps_button_inactive'
    MAIN_AUDIO_BUTTON_ACTIVE = 'main_audio_button_active'
    MAIN_AUDIO_BUTTON_INACTIVE = 'main_audio_button_inactive'
    MAIN_CLIMATE_BUTTON_ACTIVE = 'main_climate_button_active'
    MAIN_CLIMATE_BUTTON_INACTIVE = 'main_climate_button_inactive'
    MAIN_NAVIGATION_BUTTON_INACTIVE = 'main_navigation_button_inactive'
    MAIN_PHONE_BUTTON_ACTIVE = 'main_phone_button_active'
    MAIN_PHONE_BUTTON_INACTIVE = 'main_phone_button_inactive'
    MAIN_SETTINGS_BUTTON_ACTIVE = 'main_settings_button_active'
    MAIN_SETTINGS_BUTTON_INACTIVE = 'main_settings_button_inactive'
    MEDIUM_BUTTON = 'medium_button'
    PHONE_CONNECT_TO_BUTTON = 'phone_connect_to_button'
    PHONE_PAIR_DISCOVER_BUTTON = 'phone_pair_discover_button'
    PHONE_PAIR_PHONE_BUTTON = 'phone_pair_phone_button'
    RESET_ALL_BUTTON = 'reset_all_button'
    SCROLL = 'scroll'
    SETTINGS_AUTOMATIC_UPDATES_BUTTON = 'settings_automatic_updates_button'
    SETTINGS_BLUETOOTH_BUTTON = 'settings_bluetooth_button'
    SETTINGS_CLOCK_12H_FORMAT = 'settings_clock_12h_format'
    SETTINGS_CLOCK_24H_FORMAT = 'settings_clock_24h_format'
    SETTINGS_CLOCK_BUTTON = 'settings_clock_button'
    SETTINGS_DISPLAY_BUTTON = 'settings_display_button'
    SETTINGS_GENERAL_BUTTON = 'settings_general_button'
    SETTINGS_MOBILE_APPS_BUTTON = 'settings_mobile_apps_button'
    SETTINGS_NAVIGATION_BUTTON = 'settings_navigation'
    SETTINGS_NAVIGATION_GUIDANCE_PROMPTS_CHIMES_ACTIVE = 'settings_navigation_guidance_prompts_chimes_active'
    SETTINGS_NAVIGATION_GUIDANCE_PROMPTS_PLUS = 'settings_navigation_guidance_prompts_plus'
    SETTINGS_NAVIGATION_GUIDANCE_PROMPTS_VOICE_ACTIVE = 'settings_navigation_guidance_prompts_voice_active'
    SETTINGS_NAVIGATION_GUIDANCE_PROMPTS_VOICE_INACTIVE = 'settings_navigation_guidance_prompts_voice_inactive'
    SETTINGS_RADIO_BUTTON = 'settings_radio_button'
    SETTINGS_SOUND_BALANCE_FADE_BUTTON = 'settings_sound_balance_fade_button'
    SETTINGS_SOUND_BUTTON = 'settings_sound_button'
    SETTINGS_SYNC_CONNECT_BUTTON = 'settings_sync_connect_button'
    SETTINGS_VEHICLE_BUTTON = 'settings_vehicle_button'
    SETTINGS_VOICE_CONTROL_BUTTON = 'settings_voice_control_button'
    SLIDER = 'slider'
    UNCHECKED_SETTINGS_NAVIGATION_CHECKBOX_BUTTON = 'checkbox_unchecked'


class Text:
    ADAPTIVE_VOLUME_TEXT = 'Adaptive volume'
    ADD_BLUETOOTH_DEVICE_TEXT = 'Add a Bluetooth device'
    ALL_SEATS_TEXT = 'All seats'
    ALWAYS_USE_FASTEST_ROUTE_TEXT = 'Always use Fastest route'
    AUDIO_ALERT_TEXT = 'Alert'
    AUDIO_AM_TEXT = 'AM'
    AUDIO_BLUETOOTH_AUDIO_TEXT = 'Bluetooth audio'
    AUDIO_BUTTON_PANEL_TEXT = 'Audio'
    AUDIO_CD_TEXT = 'CD'
    AUDIO_CHANNEL_GUIDE_TEXT = 'Channel guide'
    AUDIO_CLASSIC_VINYL_TEXT = 'Classic Vinyl'
    AUDIO_CLSVINYL_TEXT = 'ClsVinyl'
    AUDIO_DIRECT_TUNE_0 = '0'
    AUDIO_DIRECT_TUNE_1 = '1'
    AUDIO_DIRECT_TUNE_2 = '2'
    AUDIO_DIRECT_TUNE_3 = '3'
    AUDIO_DIRECT_TUNE_4 = '4'
    AUDIO_DIRECT_TUNE_5 = '5'
    AUDIO_DIRECT_TUNE_6 = '6'
    AUDIO_DIRECT_TUNE_7 = '7'
    AUDIO_DIRECT_TUNE_8 = '8'
    AUDIO_DIRECT_TUNE_9 = '9'
    AUDIO_DIRECT_TUNE_CANCEL = 'Cancel'
    AUDIO_DIRECT_TUNE_ENTER = 'Enter'
    AUDIO_DIRECT_TUNE_TEXT = 'Direct tune'
    AUDIO_FM_TEXT = 'FM'
    AUDIO_FREQUENCY_107_9_Mhz = '107.9 MHz'
    AUDIO_FREQUENCY_1710 = '1710'
    AUDIO_FREQUENCY_TEXT = 'Frequency'
    AUDIO_HIGHWAY_TEXT = 'Highway'
    AUDIO_PRESETS_TEXT = 'Presets'
    AUDIO_REPLAY_TEXT = 'Replay'
    AUDIO_ROCK_TEXT = 'Rock'
    AUDIO_SIRIUSXM_TEXT = 'SiriusXM'
    AUDIO_SOURCES_TITLE_TEXT = 'Audio sources'
    AUTO_TEXT = 'AUTO'
    AVOID_FERRIES_CAR_TRAINS_TEXT = 'Avoid ferries/car trains'
    AVOID_MOTORWAYS_TEXT = 'Avoid motorways'
    AVOID_TOLL_ROADS_TEXT = 'Avoid toll roads'
    BLUETOOTH_SETTINGS_TEXT = 'Bluetooth settings'
    BLUETOOTH_VIEW_DEVICES_TEXT = 'View devices'
    BREADCRUMBS = 'Breadcrumbs'
    CANCEL = 'Cancel'
    CHIMES_ONLY_TEXT = 'Chimes only'
    CITY_MODEL_TEXT = '3D city model'
    CLIMATE_A_C_CONTROLS_TEXT = 'A/C controls'
    CLIMATE_A_C_TEXT = 'A/C'
    CLIMATE_BUTTON_PANEL_TEXT = 'Climate'
    CLIMATE_DEFROST_TEXT = 'Defrost'
    CLIMATE_MAX_TEXT = 'MAX'
    CLOCK_AUTO_TIME_ZONE_TEXT = 'Auto time zone update'
    CLOCK_FORMAT_TEXT = 'Clock format'
    CLOCK_PM_PERIOD_TEXT = 'PM'
    CLOCK_RESET_GPS_TEXT = 'Reset clock to GPS time'
    CLOCK_SETTINGS_TITLE_TEXT = 'Clock settings'
    CLOSE_TEXT = 'Close'
    CONNECT_A_DEVICE_TEXT = 'Connect a device'
    CONNECT_A_PHONE_TEXT = 'Add device'
    DEFROST_CONTROLS_TEXT = 'Defrost controls'
    DSP_OCCUPANCY_TEXT = 'DSP occupancy'
    DUAL_TEXT = 'DUAL'
    DYNAMIC_ROUTE_GUIDANCE_TEXT = 'Dynamic route guidance'
    ECO_TIME_PENALTY_TEXT = 'Eco time penalty'
    FIND_MOBILE_APPS_TEXT = 'Find'
    FIND_PARKING_AUTOMATICALLY_TEXT = 'Find parking automatically'
    GUIDANCE_PROMPTS_TEXT = 'Guidance prompts'
    HOME_FM_TEXT = 'FM'
    HOME_SIRIUS_TEXT = 'SIRIUS'
    MAP_PREFERENCES_TEXT = 'Map preferences'
    MOBILE_APPS_ADD_A_DEVICE_TEXT = 'Add a device'
    MOBILE_APPS_BUTTON_PANEL_TEXT = 'Mobile apps'
    MOBILE_APPS_FIND_MOBILE_APPS_TEXT = 'Find'
    MOBILE_APPS_SIRIUSXM_FILM_LISTINGS_TEXT = 'Film listings'
    MOBILE_APPS_SIRIUSXM_FUEL_PRICES_TEXT = 'Fuel prices'
    MOBILE_APPS_SIRIUSXM_SKI_CONDITIONS_TEXT = 'Ski conditions'
    MOBILE_APPS_SIRIUSXM_SPORTS_INFO_TEXT = 'Sports info'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_AUDIO_TEXT = 'Audio'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_CALL_TEXT = 'Call 1-888-539-SIRI'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_CHARGE_STATIONS_TEXT = 'Charge stations'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_DIAL_TEXT = 'Dial'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_FILM_TIMES_TEXT = 'Film times'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_FUEL_PRICES_TEXT = 'Fuel prices'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_NOT_AVAILABLE_TEXT = 'Not available'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_QUESTIONS_TEXT = 'Questions?'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_SKI_CONDITIONS_TEXT = 'Ski conditions'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_SPORTS_TEXT = 'Sports'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TEXT = 'Subscription info'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TITLE_TEXT = 'ESN: 075600760931'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TRAFFIC_TEXT = 'Traffic'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_WEATHER_MAP_TEXT = 'Weather map'
    MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_WEATHER_TEXT = 'Weather'
    MOBILE_APPS_SIRIUSXM_TRAFFIC_LIST_TEXT = 'Traffic list'
    MOBILE_APPS_SIRIUSXM_TRAVEL_LINK_TEXT = 'SiriusXM Travel'
    MOBILE_APPS_SIRIUSXM_WEATHER_TEXT = 'Weather'
    NAVIGATION_BUTTON_PANEL_TEXT = 'Navigation'
    NAVIGATION_FAULT_DESCRIPTION_TEXT = 'Navigation is not functioning properly.'
    NAVIGATION_FAULT_TEXT = 'Navigation fault'
    NAVIGATION_PREFERENCES_TEXT = 'Navigation preferences'
    NAVIGATION_SETTINGS_TEXT = 'Navigation settings'
    PAIR_PHONE_TEXT = 'Pair phone'
    PHONE_BEGIN_THE_PAIRING_TEXT = 'Begin the pairing process from'
    PHONE_BUTTON_PANEL_TEXT = 'Phone'
    PHONE_DISCOVER_OTHER_BLUETOOTH = 'Discover other Bluetooth devices'
    PHONE_HELP_TEXT = 'Help'
    PHONE_PAIR_A_BLUETOOTH_DEVICE_TEXT = 'Pair a Bluetooth device'
    PHONE_SEARCH_FOR_SYNC_TEXT = 'Search for SYNC'
    PHONE_YOUR_DEVICE_AND_A_PIN = 'your device and a PIN option will appear.'
    PREFERRED_ROUTE_TEXT = 'Preferred route'
    RADIO_AUTOSTORE_PRESETS_TEXT = 'Autostore presets (AST)'
    RADIO_REFRESH_TEXT = 'Refresh'
    RADIO_SETTINGS_TEXT = 'Radio settings'
    RADIO_TEXT = 'RadioText'
    ROUTE_PREFERENCES_TEXT = 'Route preferences'
    SETTING_CONNECTIVITY_SETTINGS_TEXT = 'Connectivity settings'
    SETTING_SYNC_CONNECT_TITLE_TEXT = 'SYNC Connect'
    SETTINGS_ANDROID_AUTO_PREFERENCES_TEXT = 'Android Auto'
    SETTINGS_APPLE_CARPLAY_PREFERENCES_TEXT = 'Apple CarPlay'
    SETTINGS_AUTOMATIC_SYSTEM_UPDATES_TEXT = 'Automatic system updates'
    SETTINGS_AUTOMATIC_UPDATES_TEXT = 'Automatic updates'
    SETTINGS_BLUETOOTH_ADD_DEVICE_TEXT = 'Add a Bluetooth device'
    SETTINGS_BLUETOOTH_TEXT = 'Bluetooth'
    SETTINGS_BLUETOOTH_VIEW_DEVICES = 'View devices'
    SETTINGS_BUTTON_PANEL_TEXT = 'Settings'
    SETTINGS_CLOCK_12H_TEXT = '12h'
    SETTINGS_CLOCK_24H_TEXT = '24h'
    SETTINGS_CLOCK_AM_TEXT = 'AM'
    SETTINGS_CLOCK_AUTO_TIME_ZONE_TEXT = 'Auto time zone update'
    SETTINGS_CLOCK_FORMAT_TEXT = 'Clock format'
    SETTINGS_CLOCK_PM_TEXT = 'PM'
    SETTINGS_CLOCK_RESET_CLOCK_TEXT = 'Reset clock to GPS time'
    SETTINGS_CLOCK_TEXT = 'Clock'
    SETTINGS_DISPLAY_AUTO_DIM_TEXT = 'Auto dim'
    SETTINGS_DISPLAY_BACKGROUND_TEXT = 'Background'
    SETTINGS_DISPLAY_BRIGHTNESS_TEXT = 'Brightness'
    SETTINGS_DISPLAY_MODE_TEXT = 'Mode'
    SETTINGS_DISPLAY_OFF_TEXT = 'Display off'
    SETTINGS_DISPLAY_SETTINGS_TITLE_TEXT = 'Display settings'
    SETTINGS_DISPLAY_TEXT = 'Display'
    SETTINGS_ENABLE_MOBILE_APPS_VIA_USB_TEXT = 'Enable Mobile apps via USB'
    SETTINGS_GENERAL_ABOUT_SYNC_TEXT = 'About SYNC'
    SETTINGS_GENERAL_LANGUAGE_TEXT = 'Language'
    SETTINGS_GENERAL_MASTER_RESET_TEXT = 'Master reset'
    SETTINGS_GENERAL_MEASURE_UNIT_TEXT = 'Measure unit'
    SETTINGS_GENERAL_SETTINGS_LABEL_TEXT = 'General settings'
    SETTINGS_GENERAL_SOFTWARE_LICENCES_TEXT = 'Software licences'
    SETTINGS_GENERAL_TEMPERATURE_UNIT_TEXT = 'Temperature unit'
    SETTINGS_GENERAL_TEXT = 'General'
    SETTINGS_GENERAL_TOUCHSCREEN_BEEP_TEXT = 'Touchscreen beep'
    SETTINGS_MEDIUM_TEXT = 'Medium'
    SETTINGS_MOBILE_APPS_SETTINGS_TITLE_TEXT = 'Mobile apps settings'
    SETTINGS_MOBILE_APPS_TEXT = 'Mobile apps'
    SETTINGS_NAVIGATION_INCEDENT_MAP_TEXT = 'Incident map icons'
    SETTINGS_NAVIGATION_POI_ICONS_TEXT = 'POI icons'
    SETTINGS_NAVIGATION_ROUTE_AVOID_TUNNELS_TEXT = 'Avoid tunnels'
    SETTINGS_NAVIGATION_TEXT = 'Navigation'
    SETTINGS_PHONE_TEXT = 'Phone'
    SETTINGS_RADIO_TEXT = 'Radio'
    SETTINGS_SOUND_ADAPTIVE_VOLUME_TEXT = 'Adaptive volume'
    SETTINGS_SOUND_BALANCE_FADE_TEXT = 'Balance/Fade'
    SETTINGS_SOUND_BASS_TEXT = 'Bass'
    SETTINGS_SOUND_MID_RANGE_TEXT = 'Mid-range'
    SETTINGS_SOUND_RESET_ALL_TEXT = 'Reset all'
    SETTINGS_SOUND_TEXT = 'Sound'
    SETTINGS_SYNC_CONNECT_TEXT = 'SYNC Connect'
    SETTINGS_VALET_MODE_TEXT = 'Valet mode'
    SETTINGS_VEHICLE_HOTSPOT_TEXT = 'Vehicle Hotspot'
    SETTINGS_VEHICLE_INFORMATION_CHIMES = 'Information chimes'
    SETTINGS_VEHICLE_ON_BOARD_MODEM_NUMBER = 'On-board modem serial number (ESN)'
    SETTINGS_VEHICLE_RESET = 'Reset'
    SETTINGS_VEHICLE_RESET_TYRE_PRESSURE = 'Reset tyre pressure'
    SETTINGS_VEHICLE_TEXT = 'Vehicle'
    SETTINGS_VEHICLE_TITLE_TEXT = 'Vehicle settings'
    SETTINGS_VEHICLE_WARNING_CHIMES = 'Warning chimes'
    SETTINGS_VIEW_CURRENT_SOFTWARE_VERSIONS_TEXT = 'View current software versions'
    SETTINGS_VOICE_CONTROL_ADVANCED_MODE_TEXT = 'Advanced mode'
    SETTINGS_VOICE_CONTROL_CALL_CONFIRMATION = 'Call confirmation'
    SETTINGS_VOICE_CONTROL_SETTINGS_TITLE_TEXT = 'Voice control settings'
    SETTINGS_VOICE_CONTROL_TEXT = 'Voice control'
    SETTINGS_VOICE_CONTROL_VOICE_COMMAND_LIST = 'Voice command lists'
    SIRIUS_XM_TRAVEL_LINK_TEXT = 'SiriusXM Travel'
    SOUND_SETTINGS_TITLE_TEXT = 'Sound settings'
    SOUND_TREBLE_TEXT = 'Treble'
    SOURCES_TEXT = 'Sources'
    VOICE_CHIMES_TEXT = 'Voice & chimes'
    VOICE_ONLY_TEXT = 'Voice only'


"""DATA FOR PARAMETERIZATION"""

# home page
home_page_text = [Text.CONNECT_A_PHONE_TEXT, Text.NAVIGATION_FAULT_TEXT, Text.AUDIO_BUTTON_PANEL_TEXT,
                  Text.CLIMATE_BUTTON_PANEL_TEXT, Text.PHONE_BUTTON_PANEL_TEXT, Text.NAVIGATION_BUTTON_PANEL_TEXT,
                  Text.MOBILE_APPS_BUTTON_PANEL_TEXT, Text.SETTINGS_BUTTON_PANEL_TEXT]

audio_page_text = [Text.SOURCES_TEXT, Text.AUDIO_FREQUENCY_TEXT, Text.AUDIO_PRESETS_TEXT,
                   Text.AUDIO_FREQUENCY_107_9_Mhz, ]

audio_sources_page_text = [Text.AUDIO_AM_TEXT, Text.AUDIO_FM_TEXT, Text.AUDIO_SIRIUSXM_TEXT,
                           Text.AUDIO_CD_TEXT, Text.AUDIO_BLUETOOTH_AUDIO_TEXT]

audio_sirius_page_text = [Text.AUDIO_CLSVINYL_TEXT, Text.AUDIO_CLASSIC_VINYL_TEXT, Text.AUDIO_ROCK_TEXT,
                          Text.AUDIO_CHANNEL_GUIDE_TEXT, Text.AUDIO_DIRECT_TUNE_TEXT, Text.AUDIO_REPLAY_TEXT,
                          Text.AUDIO_ALERT_TEXT]
audio_direct_tune_text = [Text.AUDIO_DIRECT_TUNE_1, Text.AUDIO_DIRECT_TUNE_2, Text.AUDIO_DIRECT_TUNE_3,
                          Text.AUDIO_DIRECT_TUNE_4, Text.AUDIO_DIRECT_TUNE_5, Text.AUDIO_DIRECT_TUNE_6,
                          Text.AUDIO_DIRECT_TUNE_7, Text.AUDIO_DIRECT_TUNE_8, Text.AUDIO_DIRECT_TUNE_9,
                          Text.AUDIO_DIRECT_TUNE_0, Text.AUDIO_DIRECT_TUNE_CANCEL, Text.AUDIO_DIRECT_TUNE_ENTER]

# climate page
climate_page_text = [Text.AUTO_TEXT, Text.DUAL_TEXT, Text.CLIMATE_DEFROST_TEXT, Text.CLIMATE_A_C_TEXT]
defrost_controls_page_text = [Text.DEFROST_CONTROLS_TEXT, Text.CLOSE_TEXT]
a_c_controls_page_text = [Text.CLIMATE_A_C_CONTROLS_TEXT, Text.CLIMATE_MAX_TEXT, Text.CLIMATE_A_C_TEXT]

# phone page
pair_bluetooth_device_page = [Text.PHONE_PAIR_A_BLUETOOTH_DEVICE_TEXT, Text.PHONE_HELP_TEXT,
                              Text.PHONE_SEARCH_FOR_SYNC_TEXT, Text.PHONE_BEGIN_THE_PAIRING_TEXT,
                              Text.PHONE_YOUR_DEVICE_AND_A_PIN, Text.PHONE_DISCOVER_OTHER_BLUETOOTH]

# mobile apps page
mobile_apps_page_text = [Text.MOBILE_APPS_ADD_A_DEVICE_TEXT, Text.MOBILE_APPS_FIND_MOBILE_APPS_TEXT,
                         Text.MOBILE_APPS_SIRIUSXM_TRAVEL_LINK_TEXT]
siriusxm_travel_link_text = [Text.MOBILE_APPS_SIRIUSXM_TRAFFIC_LIST_TEXT, Text.MOBILE_APPS_SIRIUSXM_FILM_LISTINGS_TEXT,
                             Text.MOBILE_APPS_SIRIUSXM_SPORTS_INFO_TEXT,
                             Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TEXT,
                             Text.MOBILE_APPS_SIRIUSXM_FUEL_PRICES_TEXT, Text.MOBILE_APPS_SIRIUSXM_WEATHER_TEXT,
                             Text.MOBILE_APPS_SIRIUSXM_SKI_CONDITIONS_TEXT]
subscription_info_text = [Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TITLE_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_AUDIO_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_TRAFFIC_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_WEATHER_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_FUEL_PRICES_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_FILM_TIMES_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_SPORTS_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_WEATHER_MAP_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_SKI_CONDITIONS_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_CHARGE_STATIONS_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_QUESTIONS_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_CALL_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_DIAL_TEXT,
                          Text.MOBILE_APPS_SIRIUSXM_SUBSCRIPTION_INFO_NOT_AVAILABLE_TEXT]

# settings page
settings_first_screen_page_text = [Text.SETTINGS_SOUND_TEXT, Text.SETTINGS_CLOCK_TEXT,
                                   Text.SETTINGS_BLUETOOTH_TEXT, Text.SETTINGS_PHONE_TEXT,
                                   Text.SETTINGS_RADIO_TEXT, Text.SETTINGS_NAVIGATION_TEXT,
                                   Text.SETTINGS_MOBILE_APPS_TEXT, Text.SETTINGS_SYNC_CONNECT_TEXT]

settings_second_screen_page_text = [Text.SETTINGS_GENERAL_TEXT, Text.SETTINGS_AUTOMATIC_UPDATES_TEXT,
                                    Text.SETTINGS_ANDROID_AUTO_PREFERENCES_TEXT,
                                    Text.SETTINGS_APPLE_CARPLAY_PREFERENCES_TEXT,
                                    Text.SETTINGS_VEHICLE_TEXT, Text.SETTINGS_DISPLAY_TEXT,
                                    Text.SETTINGS_VOICE_CONTROL_TEXT, Text.SETTINGS_VALET_MODE_TEXT]

sound_settings_page_text = [Text.SOUND_SETTINGS_TITLE_TEXT, Text.SETTINGS_SOUND_RESET_ALL_TEXT,
                            Text.SOUND_TREBLE_TEXT, Text.SETTINGS_SOUND_MID_RANGE_TEXT, Text.SETTINGS_SOUND_BASS_TEXT,
                            Text.SETTINGS_SOUND_BALANCE_FADE_TEXT, Text.SETTINGS_MEDIUM_TEXT,
                            Text.SETTINGS_SOUND_ADAPTIVE_VOLUME_TEXT, Text.DSP_OCCUPANCY_TEXT, Text.ALL_SEATS_TEXT]

clock_settings_page_text = [Text.CLOCK_SETTINGS_TITLE_TEXT, Text.SETTINGS_CLOCK_AM_TEXT, Text.SETTINGS_CLOCK_PM_TEXT,
                            Text.SETTINGS_CLOCK_FORMAT_TEXT, Text.SETTINGS_CLOCK_12H_TEXT,
                            Text.SETTINGS_CLOCK_AUTO_TIME_ZONE_TEXT, Text.SETTINGS_CLOCK_RESET_CLOCK_TEXT,
                            Text.SETTINGS_CLOCK_24H_TEXT]

bluetooth_settings_page_text = [Text.BLUETOOTH_SETTINGS_TEXT, Text.SETTINGS_BLUETOOTH_TEXT,
                                Text.SETTINGS_BLUETOOTH_ADD_DEVICE_TEXT]

radio_settings_page_text = [Text.RADIO_SETTINGS_TEXT, Text.RADIO_AUTOSTORE_PRESETS_TEXT, Text.RADIO_REFRESH_TEXT]

navigation_settings_page_text = [Text.NAVIGATION_SETTINGS_TEXT, Text.MAP_PREFERENCES_TEXT, Text.ROUTE_PREFERENCES_TEXT,
                                 Text.NAVIGATION_PREFERENCES_TEXT]

navigation_map_settings_page_text = [Text.MAP_PREFERENCES_TEXT, Text.CITY_MODEL_TEXT, Text.BREADCRUMBS,
                                     Text.SETTINGS_NAVIGATION_POI_ICONS_TEXT,
                                     Text.SETTINGS_NAVIGATION_INCEDENT_MAP_TEXT]

navigation_route_settings_page_text = [Text.ROUTE_PREFERENCES_TEXT, Text.PREFERRED_ROUTE_TEXT,
                                       Text.ALWAYS_USE_FASTEST_ROUTE_TEXT, Text.FIND_PARKING_AUTOMATICALLY_TEXT,
                                       Text.ECO_TIME_PENALTY_TEXT, Text.DYNAMIC_ROUTE_GUIDANCE_TEXT,
                                       Text.AVOID_MOTORWAYS_TEXT, Text.AVOID_TOLL_ROADS_TEXT,
                                       Text.AVOID_FERRIES_CAR_TRAINS_TEXT]

mobile_apps_settings_page_text = [Text.SETTINGS_MOBILE_APPS_SETTINGS_TITLE_TEXT,
                                  Text.SETTINGS_ENABLE_MOBILE_APPS_VIA_USB_TEXT]

sync_connect_settings_page_text = [Text.SETTING_SYNC_CONNECT_TITLE_TEXT, Text.SETTINGS_VEHICLE_HOTSPOT_TEXT,
                                   Text.SETTING_CONNECTIVITY_SETTINGS_TEXT]

general_settings_page_text = [Text.SETTINGS_GENERAL_SETTINGS_LABEL_TEXT, Text.SETTINGS_GENERAL_LANGUAGE_TEXT,
                              Text.SETTINGS_GENERAL_MEASURE_UNIT_TEXT, Text.SETTINGS_GENERAL_TEMPERATURE_UNIT_TEXT,
                              Text.SETTINGS_GENERAL_TOUCHSCREEN_BEEP_TEXT, Text.SETTINGS_GENERAL_ABOUT_SYNC_TEXT,
                              Text.SETTINGS_GENERAL_SOFTWARE_LICENCES_TEXT, Text.SETTINGS_GENERAL_MASTER_RESET_TEXT]

automatic_system_updates_settings_page_text = [Text.SETTINGS_AUTOMATIC_SYSTEM_UPDATES_TEXT,
                                               Text.SETTINGS_VIEW_CURRENT_SOFTWARE_VERSIONS_TEXT]

vehicle_settings_page_text = [Text.SETTINGS_VEHICLE_TITLE_TEXT, Text.SETTINGS_VEHICLE_ON_BOARD_MODEM_NUMBER,
                              Text.SETTINGS_VEHICLE_WARNING_CHIMES, Text.SETTINGS_VEHICLE_INFORMATION_CHIMES,
                              Text.SETTINGS_VEHICLE_RESET_TYRE_PRESSURE, Text.SETTINGS_VEHICLE_RESET]

display_settings_page_text = [Text.SETTINGS_DISPLAY_SETTINGS_TITLE_TEXT, Text.SETTINGS_DISPLAY_OFF_TEXT,
                              Text.SETTINGS_DISPLAY_BACKGROUND_TEXT, Text.SETTINGS_DISPLAY_BRIGHTNESS_TEXT,
                              Text.SETTINGS_DISPLAY_MODE_TEXT, Text.SETTINGS_DISPLAY_AUTO_DIM_TEXT]

voice_control_settings_page_text = [Text.SETTINGS_VOICE_CONTROL_SETTINGS_TITLE_TEXT,
                                    Text.SETTINGS_VOICE_CONTROL_ADVANCED_MODE_TEXT,
                                    Text.SETTINGS_VOICE_CONTROL_CALL_CONFIRMATION,
                                    Text.SETTINGS_VOICE_CONTROL_VOICE_COMMAND_LIST]

"""JIRA"""
TASK_LINK = 'https://adc.luxoft.com/jira/browse/{}'
