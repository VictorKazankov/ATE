...
def test_tap_object_by_object_data(get_state_audio_button_object_sync4):
    icon_object = get_state_audio_button_object_sync4
    # tabObject returns nothing therefore getting existing text after going to page
    hmi.tap_object(icon_object)
    assert hmi.obj_exists(Text.AUDIO_PAGE_SOURCES_TEXT)


@allure.testcase(TASK_LINK.format("VHAT-1611"), "VHAT-1611")
def test_tap_object_by_screen_point_object_data(get_state_audio_button_object_sync4):
...
def test_tap_object_by_screen_rectangle_object_data(get_state_audio_button_object_sync4):
...
def test_none_value_for_tap_object(get_state_audio_button_object_sync4):
    icon_object = get_state_audio_button_object_sync4
    assert None == hmi.tap_object(icon_object)