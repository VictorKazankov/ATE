...
@allure.testcase(constants.TASK_LINK.format("VHAT-475"), "VHAT-475")
...
def test_drag_api_horizontally(driver_sync3):
    ...
    assert helpers.get_exist_result(constants.SETTINGS_GENERAL_BUTTON)
    # return the page to previous state and check it
    helpers.touch_and_drag_page('settings_page', x=50, y=200, dx=400, dy=0)
    assert helpers.get_exist_result(constants.SETTINGS_SOUND_BUTTON)
...
def test_drag_api_vertically(driver_sync3):
    ...
    assert helpers.get_exist_result(constants.INFORMATION_DIALOG_BUTTON)
