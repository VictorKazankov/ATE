...
bool storage::JsonStorage::LoadCollection(const std::string& new_collection_type,
  ...
  if (new_collection_type == collection_type_ && new_collection_name == collection_name_) {
    logger::warn("[storage] Trying to load already-loaded collection: {}/{}", new_collection_type,
                  new_collection_name);
    return false;
  }
  ...
  try {
    ...
    for (const auto& it : jsonobj) {
      new_collection.emplace_back(it[kIconName].asString(), storage_path_ / new_collection_type / it[kIconLocation].asCString());
    }
